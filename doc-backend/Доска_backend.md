...
Kanban-доска с колонками по статусам. Пользователь перетаскивает карточки между колонками — это обновляет статус заявки. Доска работает на **LoopMatch** (матчи из направлений поиска), а не на `applications`.
...
|-------|-----|---------|----------|
| `loopId` | UUID | — | Фильтр по конкретному направлению поиска |
...
- AI-worker добавил новый матч из направления поиска → событие `card_added` появляется мгновенно
...
## 9. Фильтр по направлению поиска (client-side vs server-side)
