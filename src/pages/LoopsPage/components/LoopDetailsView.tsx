        ...
        <div className="rounded-[12px] border border-border bg-card p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] text-muted-foreground">
              {loop.selectedSources?.length ?? 0}
            </span>
          </div>
          <ul className="mt-3 flex flex-col">
            {(loop.selectedSources ?? []).slice(0, 10).map((src) => {
      ...
      <div className="overflow-hidden rounded-[12px] border border-border bg-card">
        <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
          <div>
            ...
            <div className="mt-0.5 text-[11.5px] text-muted-foreground">
              {t("loops.sourcesSubtitle", "Match counts and last run times from the backend.")}
            </div>
          </div>
        ...
        {sources.length === 0 ? (
          ...
          <ul className="divide-y divide-border">
            {sources.map((src) => {
              ...
              const label =
                DISCOVERY_SOURCE_OPTIONS.find((o) => o.value === key)?.label ?? src;
              return (
                <li
                  key={src}
                  className="grid grid-cols-[36px_minmax(0,1.6fr)_repeat(3,minmax(0,1fr))] items-center gap-4 px-5 py-3"
                >
                  <div
